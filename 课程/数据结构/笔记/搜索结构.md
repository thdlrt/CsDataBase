...
- 删除 x 之后的**维护**（必须沿结点 x 的父结点通向根的路径**反向追踪**高度的变化对路径上各个结点的影响）
  ...
  - 结点 p 的 bf 不为 0，且较矮的子树又被缩短。则在结点 p **发生不平衡**。需要进行平衡化旋转来恢复平衡。（假设高子树根为 q）
    ...
    - 如果 q 的 bf 与 p 的 bf **相同**，则执行一个单旋转来恢复平衡，结点 p 和 q 的 bf 均改为 0，同时置 shorter 为 True。**还要继续检查上层结点的平衡因子。**<img src="https://thdlrt.oss-cn-beijing.aliyuncs.com/image-20231124153430823.png" alt="image-20231124153430823" style="zoom:50%;" />
